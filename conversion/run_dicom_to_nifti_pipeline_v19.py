"""
run_dicom_to_nifti_pipeline_v18.py
...
# NOTE: Dixon rules intentionally do NOT require "dixon" anymore. Too many sequences include that word.
...
MUST_NOT_CONTAIN = {
   # "Dixon_W_COMP": ["t1map", "t2map", "moco", "grappa", "ff", "fatfraction", "cartesian", "t2s", "t2star", "t2"],
 
    #"Dixon_F_COMP": ["t1map", "t2map", "moco", "grappa", "ff", "fatfraction", "cartesian", "t2s", "t2star", "t2"],
    "Dixon_W_COMP": [],
    "Dixon_F_COMP": [],   
    "T2_Grappa_Thigh": [],
...
#MUST_CONTAIN_ANY = {
 #   "Dixon_W_COMP": ["water", "w comp", "wcomp", "w_comp"],
  #  "Dixon_F_COMP": ["fat", "f comp", "fcomp", "f_comp"],
#}

MUST_CONTAIN_ANY = {
    "Dixon_W_COMP": ["composed w comp", "w comp filt", "w comp"],
    "Dixon_F_COMP": ["composed f comp", "f comp filt", "f comp"],
    "T2_Grappa_Thigh": [],
...
def normalize_header_text(protocol_name: Optional[str], series_desc: Optional[str]) -> str:
    """
    ...
    """
    s = f"{protocol_name or ''} {series_desc or ''}".lower()
    s = s.replace("*", "star")
    s = re.sub(r"[^a-z0-9]+", " ", s)
...
def build_series_index(staging_dir: Path):
    ...
    for uid, rec in series.items():
        rec["norm_text"] = normalize_header_text(rec.get("ProtocolName"), rec.get("SeriesDescription"))
...
def pick_series(series_dict):
    ...
    for outname, keyword_groups in RULES.items():
        ...
        for uid, rec in series_dict.items():
            ...
            if "dixon" in text.lower():
                if "water" in text.lower() or "w_comp" in text.lower() or "wcomp" in text.lower():
                    selected["Dixon_W_COMP"] = (uid, rec)
                    continue
                if "fat" in text.lower() or "f_comp" in text.lower() or "fcomp" in text.lower():
                    selected["Dixon_F_COMP"] = (uid, rec)
                    continue
        ...
        if final_path.exists():
            try:
                do_collapse = (COLLAPSE_SCOPE.lower() == "all") or (
                    COLLAPSE_SCOPE.lower() == "seg_only" and outname in ("Dixon_W_COMP", "Dixon_F_COMP")
                )
                if do_collapse:
...
# NOTE: Could not auto-insert MOCO guard; please insert manually.
