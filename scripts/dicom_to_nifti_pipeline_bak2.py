...
RULES: Dict[str, List[List[str]]] = {
    ...
    # Newer protocols acquire a single combined T2 TSE 448 series covering thigh+calf
    # instead of separate GRAPPA thigh/calf series (no "grappa" or "thigh"/"calf" token
    # in the DICOM headers at all, e.g. ProtocolName "t2_tse_tra_448"). Matched
    # separately from T2_Grappa_Thigh/Calf above so studies with real split series
    # keep using those; this is only a fallback for the combined-series case.
    "T2_448_Combined": [
        ["t2", "tse", "448"],
        ["t2", "tse", "tra"],
    ],
...
MUST_NOT_CONTAIN = {
    ...
    "T2_Grappa_Thigh": [],
    "T2_Grappa_Calf": [],
    # Exclude the sagittal L-spine T2 TSE and any T2 mapping/MOCO series, which use
    # different wording but could otherwise collide with loose "t2"+"tse" matching.
    "T2_448_Combined": ["sag", "spine", "map", "moco"],
}
...
# T2_Grappa_Thigh/T2_Grappa_Calf (split series) and T2_448_Combined (single combined
# series) are alternative ways of satisfying the T2 requirement, not both-required.
T2_ALTERNATIVE_OUTPUTS = [
    {"T2_Grappa_Thigh", "T2_Grappa_Calf"},
    {"T2_448_Combined"},
]


def missing_outputs(found: set) -> List[str]:
    """RULES keys not present in `found`, treating T2 alternatives as either/or."""
    non_t2_keys = [k for k in RULES.keys() if k not in {"T2_Grappa_Thigh", "T2_Grappa_Calf", "T2_448_Combined"}]
    missing = [k for k in non_t2_keys if k not in found]
    if not any(alt <= found for alt in T2_ALTERNATIVE_OUTPUTS):
        missing.append("T2_Grappa_Thigh/T2_Grappa_Calf or T2_448_Combined")
    return missing


def already_converted(out_dir: Path) -> bool:
    found = {outname for outname in RULES.keys() if (out_dir / f"{outname}.nii.gz").exists()}
    for outname in RULES.keys():
        if outname in {"T2_Grappa_Thigh", "T2_Grappa_Calf", "T2_448_Combined"}:
            continue
        if outname not in found:
            return False
    if not any(alt <= found for alt in T2_ALTERNATIVE_OUTPUTS):
        return False
    return True
...
def process_session(session_dir: Path) -> Dict:
    ...
    print(f"    DICOM staging folder: {staging}")
    ...
    missing = missing_outputs(set(selected.keys()))
